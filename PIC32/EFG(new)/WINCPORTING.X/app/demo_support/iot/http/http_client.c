...
//#include "../framework/driver/winc1500/winc1500_api.h"
...
#define RECV_BUF_SIZE BUFFER_MAX_SIZE  //256
#define SEND_BUF_SIZE MIN_SEND_BUFFER_SIZE
char tmp_httpRecvBuf[RECV_BUF_SIZE];
void http_client_get_config_defaults(struct http_client_config *const config)
{
    ...
    config->recv_buffer_size = RECV_BUF_SIZE;//256;
    config->send_buffer_size = SEND_BUF_SIZE;//MIN_SEND_BUFFER_SIZE;
...
int http_client_init(struct http_client_module *const module, struct http_client_config *config)
{
    ...
    {
        // ToDo: use malloc
        module->config.recv_buffer = tmp_httpRecvBuf ;//malloc(config->recv_buffer_size);// ToDo: use malloc
        if (module->config.recv_buffer == NULL)
        {
            return -ENOMEM;
        }
...
int http_client_deinit(struct http_client_module *const module)
{
    ...
    {
        module->config.recv_buffer = 0;  //free(module->config.recv_buffer); //ToDo: use malloc free       
    }

    if (module->req.ext_header != NULL)
    {
        module->req.ext_header = NULL; // free(module->req.ext_header); //ToDo: use malloc free 
    }
...
{
    t_socketConnect *msg_connect;
    t_socketRecv *msg_recv;
    int16_t send_ret;
    ...
    case M2M_SOCKET_CONNECT_EVENT:
        msg_connect = (t_socketConnect*)msg_data;
        data.sock_connected.result = msg_connect->error;
        if (msg_connect->error < 0)
        {
            /* Remove reference. */
            _http_client_clear_conn(module, _hwerr_to_stderr(msg_connect->error));
        }
        else
        {
    ...
    case M2M_SOCKET_RECV_EVENT:
        msg_recv = (t_socketRecv*)msg_data;
        /* Start post processing. */
        if (msg_recv->bufSize > 0)
        {
            _http_client_recved_packet(module, msg_recv->bufSize);
        }
        else
        {
            /* Socket was occurred errors. Close this session. */
            _http_client_clear_conn(module, _hwerr_to_stderr(msg_recv->bufSize));
        }
    ...
    enum http_method method, struct http_entity *const entity, const char *ext_header)
{
    ...
    if (module->req.ext_header != NULL) 
    {
        module->req.ext_header = NULL; // free(module->req.ext_header); // ToDo: will use malloc free
    }
    if (ext_header != NULL) 
    {
#if !defined(__XC8)  // pic18 cannot support strdup()
    ...
    case STATE_INIT:
        ...
        if (module->sock >= 0) 
        {
            ...
            if (_is_ip(module->host)) 
            {
                addr_in.sin_family = AF_INET;
                addr_in.sin_port = _htons(module->config.port);
                inet_pton4((char *)module->host, &(addr_in.sin_addr.s_addr));
                connect(module->sock, (struct sockaddr *)&addr_in, sizeof(struct sockaddr_in));
            } 
            else 
            {
...
int _http_client_send_wait(void *_module, char *buffer, size_t buffer_len)
{
    ...
    {
#if defined(__XC8)
        m2m_wifi_task_Pic18WaiteHttpSend(); 
#else
        m2m_wifi_task();
#endif
#if defined(TODO)        
...
#define HTTP_CHUNKED_MAX_LENGTH 3 /*TCP MTU is 1400(0x578) */
    // ToDo: use malloc
    char buffer[SEND_BUF_SIZE];//[module->config.send_buffer_size];
...
#if defined(__XC8)
void http_client_socket_event_handler_Pic18WaiteHttpSend(SOCKET sock, uint8_t msg_type, void *msg_data)
{
    /* Find instance using the socket descriptor. */
    struct http_client_module *module = module_ref_inst[sock];
    /* If cannot found reference, This socket is not HTTP client socket. */
    ...
    if (msg_type == M2M_SOCKET_SEND_EVENT)
    {
        /* Disable sending flag. */
        module->sending = 0;
    }
